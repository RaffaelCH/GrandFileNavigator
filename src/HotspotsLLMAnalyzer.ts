...
export class HotspotLLMAnalyzer {
  ...
  private static async queryLLM(
    ...
    try {
      const response = await axios.post(
        "http://llm.hasel.dev:20769/v1/chat/completions",
        {
          model: "openai-gpt-4o",
      ...
      if (result.choices && result.choices.length > 0) {
        ...
        return {
          ...
          relevance: jsonResponse.relevance,
        };
      }
  ...
  private static async extractSurroundingSection(
    ...
    if (enclosingSymbol) {
      console.log(
        `Enclosing symbol found: ${enclosingSymbol.name} (${enclosingSymbol.kind})`
      );
      return {
      ...
      console.warn(
        `No enclosing symbol found for hotspot at lines ${startLine}-${endLine}`
      );
      return { start: startLine, end: endLine };
    }
  }
  ...
  public static async analyzeHotspotWithLLM(
    ...
    console.log(
      `Starting analysis for hotspot: ${filePath} (${startLine}-${endLine})`
    ...
    if (existingHotspot) {
      vscode.window.showInformationMessage(
        `Hotspot at ${filePath}:${startLine}-${endLine} is already analyzed. Skipping...`
      );
      return; // Skip if already analyzed
    }

    // Extract surrounding section for the hotspot
    const { start: surroundingStartLine, end: surroundingEndLine } =
      await this.extractSurroundingSection(document, startLine, endLine);
    ...
    const surroundingSection = document.getText(
      new vscode.Range(
        ...
        new vscode.Position(
          surroundingEndLine,
          document.lineAt(surroundingEndLine).text.length
        )
      )
    );
    ...
    try {
      ...
      if (llmResult.summary && llmResult.relevance) {
        ...
        console.warn(
          `Received invalid LLM result for hotspot at ${filePath}:${startLine}-${endLine}`
        );
      }
    } catch (error) {
      console.error(
        `Error analyzing hotspot at ${filePath}:${startLine}-${endLine}:`,
        error
      );
    }

    console.log(
      `Finished analysis for hotspot: ${filePath} (${startLine}-${endLine})`
  ...
  private static async saveLLMHotspotData(context: vscode.ExtensionContext) {
    try {
      ...
      if (storageUri) {
        ...
        const llmHotspotFilePath = path.join(
          storagePath,
          HotspotLLMAnalyzer.llmHotspotsFilename
        ); // Use the correct reference

        // Write data to llmHotspots.json
        writeFileSync(
          llmHotspotFilePath,
          JSON.stringify(llmHotspotData, null, 2)
        );
        //vscode.window.showInformationMessage(`LLM Hotspots Data saved at ${llmHotspotFilePath}.`);
      } else {
        vscode.window.showErrorMessage(
          "Error: Unable to find storage location for llmHotspots.json."
        );
      }
    } catch (error) {
      vscode.window.showErrorMessage(
        `Error saving LLM Hotspots data: ${(error as Error).message}`
      );
    }
  }
  ...
  private static async processQueue(
    ...
    while (hotspotQueue.length > 0) {
      ...
      if (hotspot) {
        try {
          await HotspotLLMAnalyzer.analyzeHotspotWithLLM(
            hotspot,
            document,
            context
          );
